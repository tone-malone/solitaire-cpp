...
struct Animation
{
    AnimType type = MoveCard;
    Card card;
    int srcX, srcY, dstX, dstY;
    Uint32 startTime, duration;
    // what to do when this anim finishes:
...
public:
    ...
    {
        bool over = pointInRect(mouseX, mouseY, rect.x, rect.y, rect.w, rect.h);
        if (over && mouseDown)
            state = Pressed;
        else if (over)
            state = Hovered;
    ...
    void render(SDL_Renderer *renderer, TTF_Font *font)
    {
        ...
        SDL_Color bg = (state == Pressed   ? SDL_Color{100, 100, 250, 255}
                        : state == Hovered ? SDL_Color{180, 180, 255, 255}
...
public:
    ...
    void drawPips(int x, int y, int width, int height, int cardValue, int suit)
    {
        ...
        {
        case 0:
            pipTexture = mSpadeTexture;
            break;
        case 1:
            pipTexture = mHeartTexture;
            break;
        case 2:
            pipTexture = mDiamondTexture;
            break;
        case 3:
            pipTexture = mClubTexture;
            break;
        default:
            break;
        }
...
// Can a single card go onto a tableau pile?
bool canPlaceOnTableau(const Card &card, const Pile &pile)
{
    if (pile.cards.empty())
        return (card.value == 13); // only King on empty
    const Card &top = pile.cards.back();
    bool oppositeColor = (isRedSuit(card.suit) != isRedSuit(top.suit));
    return oppositeColor && (card.value == top.value - 1);
}

// Can an entire dragged sequence go onto a tableau pile?
bool canMoveSequence(const std::vector<Card> &seq, const Pile &pile)
{
    // 1) internal sequence must be valid (descending, alternating)
    for (size_t i = 1; i < seq.size(); ++i)
    {
        const Card &above = seq[i - 1], &below = seq[i];
        bool altColor = (isRedSuit(above.suit) != isRedSuit(below.suit));
        if (!altColor || above.value != below.value + 1)
            return false;
    }
    // 2) the top of that sequence must be placeable on the pile
    return canPlaceOnTableau(seq.front(), pile);
}
...
void updateAnimations(CardRenderer &renderer)
{
    ...
    for (size_t i = 0; i < animations.size();)
    {
        ...
        {
            int x = lerp(A.srcX, A.dstX, e);
            int y = lerp(A.srcY, A.dstY, e);
            renderer.drawCard(x, y, A.card);
        }
...
public:
    ...
    void animateAutoMove(int sourcePileIndex, int cardIndex, int destPileIndex,
                         int srcX, int srcY, int destX, int destY)
    {
        Card c = mGame.piles[sourcePileIndex].cards[cardIndex];
        mGame.piles[sourcePileIndex].cards.erase(
            mGame.piles[sourcePileIndex].cards.begin() + cardIndex);

        // capture sourcePileIndex so we can flip its new top card
        animateCardMove(c, srcX, srcY, destX, destY, 500,
                        [this, c, sourcePileIndex, destPileIndex]()
                        {
                            // 1) place onto foundation
                            mGame.piles[destPileIndex].cards.push_back(c);
                            mGame.score += 10;
                            mGame.moveCount++;

                            // 2) flip the new top of the origin pile, if any
                            auto &orig = mGame.piles[sourcePileIndex];
                            if (!orig.cards.empty() && !orig.cards.back().faceUp)
                                orig.cards.back().faceUp = true;

                            // 3) play sound and check win
    ...
    {
        // --- check Waste as before ---
        Pile &waste = mGame.piles[1];
        if (!waste.cards.empty() && waste.cards.back().faceUp)
        {
            for (int f = 2; f < 6; ++f)
            {
                if (mGame.canPlaceOnFoundation(waste.cards.back(), mGame.piles[f]))
                {
                    hintPileIndex = 1;
                    hintCardIndex = (int)waste.cards.size() - 1;
                    destIndex = f;
                    return true;
                }
            }
        ...
        // --- only top of each tableau ---
        for (int i = 6; i < (int)mGame.piles.size(); ++i)
        {
            Pile &pile = mGame.piles[i];
            if (!pile.cards.empty())
            {
                int topIdx = (int)pile.cards.size() - 1;
                Card &c = pile.cards[topIdx];
                if (c.faceUp)
                {
                    for (int f = 2; f < 6; ++f)
                    {
                        if (mGame.canPlaceOnFoundation(c, mGame.piles[f]))
                        {
                            hintPileIndex = i;
                            hintCardIndex = topIdx;
                            destIndex = f;
                            return true;
                        }
                    }
    ...
    void render()
    {
        ...
        bool down = (mbtn & SDL_BUTTON(SDL_BUTTON_LEFT)) != 0;
        for (auto &btn : mMenuButtons)
            btn.update(mx, my, down);
        for (auto &btn : mSettingsButtons)
            btn.update(mx, my, down);
        for (auto &btn : mPlayingButtons)
            btn.update(mx, my, down);
        for (auto &btn : mStatisticsButtons)
            btn.update(mx, my, down);
